...
function display_maker_json() {
	...
	if ( '/maker.json' === $request || '/maker.json?' === substr( $request, 0, 11 ) ) {
		...
		if ( ! empty( $post_id ) ) {
			...
			$makerjson = $post->post_content;

			/**
			 * Filter the maker.json content.
			 *
			 * @since 1.0.0
			 *
			 * @param string $makerjson The existing maker.json content.
			 */
			echo esc_html( apply_filters( 'maker_json_content', $makerjson ) );
			die();
		}
	}
}
